...
use kimichat_models::{Usage, ChatRequest, StreamChunk};
use kimichat_agents::agent::ToolDefinition;
use crate::{log_request, log_request_to_file, log_response, log_stream_chunk};
use crate::tools_execution::parse_xml_tool_calls;
use crate::{ToolCall, FunctionCall};
use kimichat_agents::agent::ChatMessage;
...
pub async fn call_api_streaming_with_llm_client(
    chat: &KimiChat,
    messages: &[Message],
    model: &ModelType,
) -> Result<(Message, Option<Usage>, ModelType)> {
