...
use kimichat_agents::progress_evaluator::{ProgressEvaluator, ToolCallInfo};
                    ...
                    result = async {
                        if chat.stream_responses {
                            ...
                            if should_use_anthropic {
                                ...
                                crate::call_api_streaming_with_llm_client(chat, &chat.messages, &chat.current_model).await
                            } else {
                                // Use old streaming for OpenAI-compatible APIs
                                crate::call_api_streaming(chat, &chat.messages).await
                            }
                        } else {
                            crate::call_api(chat, &chat.messages).await
                        }
                    } => result?,
                    _ = token.cancelled() => {
