...
use kimichat_models::{Usage, ChatRequest, ChatResponse};
use kimichat_agents::ToolDefinition;
use crate::{log_request, log_request_to_file, log_response};
...
pub async fn call_api_with_llm_client(
    chat: &KimiChat,
    messages: &[Message],
    model: &ModelType,
) -> Result<(Message, Option<Usage>, ModelType)> {
