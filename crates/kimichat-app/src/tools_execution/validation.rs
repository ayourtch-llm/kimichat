...
use crate::KimiChat;
use kimichat_types::{ModelType, Message, ToolCall, FunctionCall};
use kimichat_models::{ChatRequest, ChatResponse};
use crate::log_request_to_file;

/// Repair a malformed tool call using AI to fix the JSON arguments
pub async fn repair_tool_call_with_model(
    chat: &KimiChat,
    tool_call: &ToolCall,
    error_msg: &str,
) -> Result<ToolCall> {
...
pub fn validate_and_fix_tool_calls_in_place(chat: &mut KimiChat) -> Result<bool> {
