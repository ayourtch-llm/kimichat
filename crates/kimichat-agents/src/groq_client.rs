use crate::agent::{LlmClient, LlmResponse, ChatMessage, ToolDefinition};
...
impl LlmClient for GroqLlmClient {
    async fn chat(&self, messages: Vec<ChatMessage>, tools: Vec<ToolDefinition>) -> Result<LlmResponse> {
        ...
        let message = if let Some(choice) = chat_response.choices.into_iter().next() {
            ChatMessage {
                ...
                tool_calls: choice.message.tool_calls.map(|calls| {
                    calls.into_iter().map(|call| crate::agent::ToolCall {
                        id: call.id,
        ...
        Ok(LlmResponse {
            message,
            usage: chat_response.usage.map(|usage| crate::agent::TokenUsage {
...
impl GroqLlmClient {
    ...
    async fn build_chat_request(&self, messages: Vec<ChatMessage>, tools: Vec<ToolDefinition>) -> Result<serde_json::Value> {
        let chat_messages: Vec<kimichat_types::Message> = messages.into_iter().map(|msg| {
            kimichat_types::Message {
                role: msg.role,
                content: msg.content,
                tool_calls: msg.tool_calls.map(|calls| {
                    calls.into_iter().map(|call| kimichat_types::ToolCall {
                        id: call.id,
                        tool_type: "function".to_string(),
                        function: kimichat_types::FunctionCall {
                            name: call.function.name,
                            arguments: call.function.arguments,
                        },
                    }).collect()
